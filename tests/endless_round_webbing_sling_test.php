...
$inspection = [
    ...
    'client_name'=>'Riverside Fabrication Services','client_address'=>'Industrial Layout, Port Harcourt',
...
$values = [
    ...
    'equipment_type'=>'Endless Round Webbing Sling','length'=>'4 m','equipment_description'=>'Polyester endless round sling with protective cover',
    'manufacturer'=>'West African Lifting Products','equipment_id_number'=>'RFS-RS-042','asset_number'=>'ASSET-042',
    'standard'=>'BS EN 1492-2','safe_working_load'=>'5 tonnes','date_of_manufacture'=>'2024-03-10','date_of_last_examination'=>'2026-01-12',
...
$failed = 0;
$ready = endless_round_webbing_sling_readiness($inspection,$rows,'JUVA/RFS/ERWSLNG/001','2026-07-14','2027-01-14',[],false,0);
if (!$ready['ready']) { echo "ERS OPTIONAL EVIDENCE READINESS: FAIL\n"; print_r($ready); $failed++; } else echo "ERS OPTIONAL EVIDENCE READINESS: PASS\n";
...
foreach (['first_examination_after_installation','installed_correctly','examined_within_six_months','examined_within_twelve_months','examined_under_scheme','examined_after_exceptional_circumstances','fit_for_purpose'] as $key) {
    foreach (['Yes','No'] as $answer) {
        ...
        $check=endless_round_webbing_sling_readiness($inspection,$copy,'JUVA/RFS/ERWSLNG/001','2026-07-14','2027-01-14',[],false,0);
        if(!$check['ready']){$failed++;echo "ERS BOOLEAN $key=$answer: FAIL\n";}
    }
}
if ($failed===0) echo "ERS BOOLEAN YES/NO MATRIX: PASS\n";

foreach ([[true,'Yes'],[false,'No'],[1,'Yes'],[0,'No'],['1','Yes'],['0','No'],['yes','Yes'],['no','No']] as $case) {
    if (endless_round_webbing_sling_boolean($case[0]) !== $case[1]) {
        echo 'ERS BOOLEAN NORMALIZATION '.var_export($case[0],true).": FAIL\n";
        $failed++;
    }
}
if (endless_round_webbing_sling_boolean(null) !== null || endless_round_webbing_sling_boolean('') !== null) {
    echo "ERS BOOLEAN UNANSWERED NORMALIZATION: FAIL\n";
    $failed++;
} else echo "ERS BOOLEAN REPRESENTATION NORMALIZATION: PASS\n";

$missingAnswer = $rows;
foreach ($missingAnswer as &$row) if ($row['field_key'] === 'examined_within_twelve_months') $row['value_text'] = '';
unset($row);
$missingCheck = endless_round_webbing_sling_readiness($inspection,$missingAnswer,'JUVA/RFS/ERWSLNG/001','2026-07-14','2027-01-14',[],false,0);
...
if (($blocker['key'] ?? '') !== 'examined_within_twelve_months' || ($blocker['step'] ?? 0) !== 3 || empty($blocker['label']) || empty($blocker['reason'])) {
    echo "ERS STRUCTURED BLOCKER: FAIL\n";
    print_r($missingCheck);
    $failed++;
} else echo "ERS STRUCTURED BLOCKER: PASS\n";
$futureDanger = $rows;
...
unset($row);
$dangerCheck = endless_round_webbing_sling_readiness($inspection,$futureDanger,'JUVA/RFS/ERWSLNG/001','2026-07-14','2027-01-14',[],false,0);
if (!isset($dangerCheck['validation_errors']['action_due_date'])) { echo "ERS CONDITIONAL DEFECT DATE: FAIL\n"; $failed++; } else echo "ERS CONDITIONAL DEFECT DATE: PASS\n";

$auditDir = ensure_private_storage_dir('audit/endless-round-webbing-sling/readiness-fix/run-' . gmdate('Ymd-His'));
$pdf = $auditDir . DIRECTORY_SEPARATOR . 'non-aveon-endless-round-webbing-sling-readiness-fix.pdf';
$payload=endless_round_webbing_sling_payload($inspection,$rows,'JUVA/RFS/ERWSLNG/001',1,'https://cert.juvaoil.com/verify/0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef','2026-07-14','2027-01-14');
$payload['authentication']=['inspector_signature_path'=>null,'authenticator_signature_path'=>null,'company_stamp_path'=>null];
$payload['status']='VALID';
@unlink($pdf);
endless_round_webbing_sling_render_certificate_pdf($pdf,$payload);
if(!is_file($pdf)||filesize($pdf)<100000||strpos((string)file_get_contents($pdf),'/Count 1')===false){echo "ERS PDF RENDER: FAIL\n";$failed++;}else echo "ERS PDF RENDER: PASS\n";
...
$reference='C:/Users/JUDE/Documents/Juva Oil/BAR CODE CERTIFICATE SAMPLES/ENDLESS ROUND  WEBBING SLING.jpg';
if(extract_pdf_jpeg($pdf,$tmpJpg)&&is_file($reference)){
    ...
    $diff=imagecreatetruecolor($w,$h);$sumDiff=0;for($y=0;$y<$h;$y++){for($x=0;$x<$w;$x++){ $a=imagecolorat($refSized,$x,$y);$b=imagecolorat($genSized,$x,$y);$r=abs((($a>>16)&255)-(($b>>16)&255));$g=abs((($a>>8)&255)-(($b>>8)&255));$bl=abs(($a&255)-($b&255));$sumDiff+=($r+$g+$bl);imagesetpixel($diff,$x,$y,(($r << 16) | ($g << 8) | $bl)); }}imagepng($diff,$auditDir.DIRECTORY_SEPARATOR.'pixel-difference.png');echo 'ERS VISUAL MEAN ABSOLUTE DIFFERENCE: '.number_format($sumDiff/($w*$h*3),2)."\n";
    ...
    echo "ERS VISUAL ARTIFACTS: PASS\n";
}else{echo "ERS VISUAL ARTIFACTS: FAIL\n";$failed++;}
