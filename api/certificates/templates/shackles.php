...
function shackles_render_certificate_pdf(string $path, array $p): void
{
    ...
    $put($im,$asset.'/leea-logo.png',65,35,175,120); $put($im,certificate_runtime_asset('logo.png'),1940,20,330,190);
    $rounded($im,320,60,1900,155,18); $text($im,'JUVA-OIL SERVICES (NIG) LIMITED',1168,128,42,$bold,'center');
    $text($im,'CERTIFICATE OF THOROUGH EXAMINATION',1168,215,35,$bold,'center');
    $text($im,'SHACKLES',1168,255,27,$bold,'center');
    foreach(["This report complies with the Lifting Equipment Engineers Association's technical requirements",'The Lifting Operations and Lifting Equipment Regulations 1998 SI NO.2307','The Supply of Machinery (Safety) Regulation 1992 SI NO.3073','The Provision and Use of Work Equipment Regulations 1998 SI NO.2306','Nigeria Factories Act CAP F1 LFN, 2004'] as $i=>$r)$text($im,$r,1168,288+($i*24),17,$font,'center');
    ...
    $cell($im,$x,$y,$w,52,'REASON FOR EXAMINATION    INSTALLATION: A     6 MONTHLY: B     12 MONTHLY: C     WRITTEN SCHEME: D     EXCEPTIONAL CIRCUMSTANCE: E',17,$bold);$y+=70;
