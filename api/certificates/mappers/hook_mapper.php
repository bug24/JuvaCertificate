...
function hook_readiness(array $inspection, array $rows, array $items, string $number, string $issuedAt, string $expiry): array
{
    ...
    foreach ($itemRows as $index=>$item) {
        ...
        foreach (['last_thorough_examination_date','next_thorough_examination_date'] as $key) if (($item[$key]??'')!=='' && !valid_iso_date((string)$item[$key])) $errors['hook_items.'.($index+1).'.'.$key] = 'Row '.($index+1).' has an invalid '.str_replace('_',' ',$key).'.';
