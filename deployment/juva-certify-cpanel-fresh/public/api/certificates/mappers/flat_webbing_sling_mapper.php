...
function flat_webbing_sling_field_map(array $rows): array
{
    $map = [];
    foreach ($rows as $row) {
        ...
        $map[$key] = in_array($key, $booleanKeys, true) ? (flat_webbing_sling_boolean($raw) ?? trim((string) $raw)) : trim((string) $raw);
    }
    return $map;
}

function flat_webbing_sling_boolean($value): ?string
{
...
function flat_webbing_sling_blocker(string $type, string $key, string $label, string $section, string $reason, int $step = 3): array
{
    return ['type' => $type, 'key' => $key, 'field_key' => $key, 'label' => $label, 'section' => $section, 'step' => $step, 'reason' => $reason];
}
function flat_webbing_sling_readiness(array $inspection, array $rows, string $number, string $issuedAt, string $expiry, array $attachments = [], bool $requireEvidence = false, int $minimumEvidenceFiles = 0): array
{
    $fields = flat_webbing_sling_field_map($rows);
    $missing = [];
    $errors = [];
    foreach ($rows as $row) {
        if ((int) ($row['is_required'] ?? 0) === 1 && (($row['value_text'] ?? null) === null || (is_string($row['value_text']) && trim($row['value_text']) === ''))) {
            $missing[] = flat_webbing_sling_blocker('field', (string) $row['field_key'], (string) $row['label'], 'Inspection fields', 'No value has been saved.');
        }
    }
    foreach ([
        ...
        'inspection_location' => ['Address of Premises Where Examination Was Made', (string) ($inspection['location'] ?? '')],
    ] as $key => $item) {
        if (trim($item[1]) === '') {
            $missing[] = flat_webbing_sling_blocker('field', $key, $item[0], $key === 'certificate_number' ? 'Records' : 'Details', 'No value has been saved.', $key === 'certificate_number' ? 1 : 2);
        }
    }
    ...
    foreach (['report_date','date_of_manufacture','last_thorough_examination_date','action_due_date'] as $dateKey) {
    ...
    if (flat_webbing_sling_boolean($fields['defect_future_danger'] ?? null) === 'Yes') {
    ...
    if ($evidenceCount < $requiredEvidenceCount) {
        $missing[] = flat_webbing_sling_blocker('evidence', 'inspection_evidence', 'Inspection Evidence Attachment', 'Evidence', 'Upload at least ' . $requiredEvidenceCount . ' evidence file(s).', 4);
    }
    $blocking = $missing;
    foreach ($errors as $key => $reason) {
        $detailsStep = in_array($key, ['inspection_date','next_due_date'], true);
        $blocking[] = flat_webbing_sling_blocker('validation', (string) $key, $booleanLabels[$key] ?? ucwords(str_replace('_', ' ', (string) $key)), $detailsStep ? 'Details' : 'Inspection fields', (string) $reason, $detailsStep ? 2 : 3);
    }
...
function flat_webbing_sling_validate(array $inspection, array $rows, string $number, string $issuedAt, string $expiry, array $attachments = [], bool $requireEvidence = false, int $minimumEvidenceFiles = 0): void
{
    $result = flat_webbing_sling_readiness($inspection, $rows, $number, $issuedAt, $expiry, $attachments, $requireEvidence, $minimumEvidenceFiles);
    if ($result['ready']) {
        return;
    }
    $labels = array_map(static function (array $field): string { return (string) $field['label']; }, $result['missing_fields']);
    $message = $labels ? "Flat Webbing Sling certificate cannot be issued. Complete:\n- " . implode("\n- ", $labels) : 'Flat Webbing Sling certificate cannot be issued. Correct the validation errors shown.';
    throw new CertificateIssueException($message, 422, $result);
}

function flat_webbing_sling_payload(array $inspection, array $rows, string $number, int $revision, string $verifyUrl, string $issuedAt, string $expiry): array
{
    global $config;
    $fields = flat_webbing_sling_field_map($rows);
    return [
        ...
        'inspection_location' => (string) ($inspection['location'] ?? ''),
        'equipment_name' => (string) ($inspection['equipment_name'] ?? 'FLAT WEBBING SLING'),
...
function flat_webbing_sling_date(string $value): string
{
