<?php
require_once __DIR__ . '/../mappers/endless_round_webbing_sling_mapper.php';

function endless_round_webbing_sling_render_certificate_pdf(string $path, array $p): void
{
    ...
    $put($im, certificate_runtime_asset('logo.png'), 85, 35, 300, 150);
    ...
    $cx = $x;
    foreach ([
        "DATE OF THOROUGH EXAMINATION\n" . endless_round_webbing_sling_date((string) $p['examination_date']),
        "DATE OF REPORT\n" . endless_round_webbing_sling_date((string) $p['report_date']),
    ...
    $cell($im, $x + $leftW + $sideW, $y, $sideW, 205, "DATE OF MANUFACTURE\n\n" . endless_round_webbing_sling_date($field('date_of_manufacture')), 16, $font, 'center');
    $cell($im, $x + $leftW + ($sideW * 2), $y, $w - $leftW - ($sideW * 2), 205, "DATE OF LAST THOROUGH EXAMINATION\n\n" . endless_round_webbing_sling_date($field('date_of_last_examination')), 15, $font, 'center');
    $y += 205;
    $checkY = $y;
    $half = (int) ($w / 2);
    ...
    $futureDangerQuestion = 'Is the defect not yet dangerous but could become a danger to persons?';
    if (($p['layout_profile'] ?? '') === 'lever_hoist' && $field('future_danger_description') !== '') {
        $futureDangerQuestion .= ' ' . strtoupper($field('future_danger_description'));
    }
    $yn($im, $x, $y, $w, 58, $futureDangerQuestion, $field('defect_future_danger')); $y += 58;
    $cell($im, $x, $y, 650, 60, 'DATE BY WHICH CORRECTIVE ACTION MUST BE COMPLETED', 17, $bold);
    $cell($im, $x + 650, $y, $w - 650, 60, endless_round_webbing_sling_date($field('action_due_date')), 18, $font, 'center'); $y += 60;
    foreach ([
    ...
    if ($y + 190 > 2295) { throw new RuntimeException('Endless Round Webbing Sling certificate content exceeds one A4 page.'); }
