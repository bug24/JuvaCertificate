...
$stmt = db()->prepare('SELECT i.*, c.name AS client_name, c.short_code AS client_short_code, e.asset_code, e.name AS equipment_name, cat.name AS category_name, cat.short_code AS category_short_code, cat.certificate_prefix, cat.identifier_label, cat.validity_months, u.name AS inspector_name, u.qualification AS inspector_qualification, au.name AS authenticator_name, au.qualification AS authenticator_qualification, src.reference AS cloned_from_reference FROM inspections i JOIN clients c ON c.id = i.client_id JOIN equipment e ON e.id = i.equipment_id JOIN certification_categories cat ON cat.id = i.category_id JOIN users u ON u.id = i.inspector_id LEFT JOIN users au ON au.id = i.authenticator_id LEFT JOIN inspections src ON src.id = i.cloned_from_inspection_id WHERE i.id = ? LIMIT 1');
$stmt->execute([$id]);
