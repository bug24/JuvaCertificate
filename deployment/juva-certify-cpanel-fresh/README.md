...
Use the consolidated clean schema and production seed supplied in `database/`; historical upgrade migrations are deliberately excluded from this fresh-install package. Use PHP 8.2+, enable `fileinfo`, GD and PDO MySQL, configure `upload_max_filesize`/`post_max_size`, and keep `display_errors=Off`.
