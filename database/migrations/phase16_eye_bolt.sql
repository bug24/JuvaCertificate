...
SET @category_id := (SELECT id FROM certification_categories WHERE code='EYEBLT' ORDER BY id LIMIT 1);

UPDATE certification_categories SET template_family='eye_bolt',layout_key='eye-bolt-landscape-v1',source_sample='EYE BOLT.jpg',certificate_template='Certificate of Thorough Examination',schema_version=2,updated_at=UTC_TIMESTAMP() WHERE id=@category_id;
...
SELECT @category_id,'Eye Bolt Thorough Examination Register',COALESCE(MAX(version),0)+1,'active',1,1,0,0,0,'image/jpeg,image/png,image/webp,application/pdf',0,0,0,0,UTC_TIMESTAMP(),UTC_TIMESTAMP() FROM form_templates WHERE category_id=@category_id;
...
(@template_id,'eye_bolt_items','Eye Bolt Examination Register','Add one row for each eye bolt covered by this certificate. Maximum six rows per one-page certificate.',1,6,'equipment_register',200,UTC_TIMESTAMP(),UTC_TIMESTAMP());
