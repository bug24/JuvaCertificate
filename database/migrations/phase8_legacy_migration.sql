...
-- Run this after importing the old localhost.sql tables into the same database.
-- Expected legacy tables include juva_client_master, juva_equipment_master,
-- juva_equipment_inspt_master, user_profile, user_access and audit_trail.

ALTER TABLE users
  ADD COLUMN legacy_reg_id VARCHAR(40) NULL AFTER client_id,
  ADD INDEX idx_users_legacy_reg_id (legacy_reg_id);

ALTER TABLE inspections
  MODIFY reference VARCHAR(150) NOT NULL,
  ADD COLUMN is_legacy TINYINT(1) NOT NULL DEFAULT 0 AFTER legacy_id,
  ADD COLUMN legacy_source_table VARCHAR(100) NULL AFTER is_legacy,
  ADD COLUMN legacy_source_reference VARCHAR(190) NULL AFTER legacy_source_table,
  ADD COLUMN legacy_mapping_status ENUM('complete','partial','raw_only') NOT NULL DEFAULT 'complete' AFTER legacy_source_reference,
  ADD COLUMN legacy_payload_json LONGTEXT NULL AFTER legacy_mapping_status,
  ADD INDEX idx_inspections_legacy (is_legacy, legacy_id);

ALTER TABLE certificates
  ADD COLUMN is_legacy TINYINT(1) NOT NULL DEFAULT 0 AFTER legacy_id,
  ADD COLUMN legacy_original_reference VARCHAR(190) NULL AFTER is_legacy,
  ADD COLUMN legacy_mapping_status ENUM('complete','partial','raw_only') NOT NULL DEFAULT 'complete' AFTER legacy_original_reference,
  ADD COLUMN legacy_payload_json LONGTEXT NULL AFTER legacy_mapping_status,
  ADD INDEX idx_certificates_legacy (is_legacy, legacy_id);

ALTER TABLE audit_logs
  ADD COLUMN legacy_id BIGINT UNSIGNED NULL AFTER entity_id,
  ADD INDEX idx_audit_legacy_id (legacy_id);
