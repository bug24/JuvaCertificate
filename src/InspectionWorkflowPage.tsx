            ...
            {filteredRows.map((row) => <tr key={row.id}>
              ...
              <td>
                <div className="row-actions wrap-actions">
                  ...
                  {can(user, "inspections.edit") && ["issued", "expired", "revoked"].includes(String(row.status)) && <button className="link-button" onClick={() => void cloneInspection(Number(row.id))}><CopyPlus size={14} />Clone / Renew</button>}
