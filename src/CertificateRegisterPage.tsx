...
export function EnhancedCertificateRegisterPage({ csrf, user, request, apiBase, onOpenInspection }: Props) {
  ...
  return <>
    ...
    {selectedRows.size > 0 && <div className="certificate-batch-bar" role="region" aria-label="Certificate batch actions"><strong>{selectedRows.size} certificate{selectedRows.size === 1 ? "" : "s"} selected</strong><span>{visibleSelected} selected on this page</span><div><button className="primary-button" disabled={batchBusy} onClick={() => void runBatch("attachment")}><Download size={16} />{batchBusy ? "Preparing..." : "Download Combined PDF"}</button><button className="secondary-button" disabled={batchBusy} onClick={() => void runBatch("inline")}><Printer size={16} />Print Selected</button><button className="link-button" disabled={batchBusy} onClick={() => setSelectedRows(new Map())}>Clear selection</button></div></div>}
